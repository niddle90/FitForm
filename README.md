...
FitForm is the client-side photo/PDF toolkit described below, plus a
...
- **Scope**: the Vault only ever requests the `drive.file` OAuth scope, which
  is Google's narrowest Drive scope. It grants access to exactly two things:
...
- **Nothing persisted except a folder ID**: the access token lives in memory
  ...
  `localStorage` is the Vault folder's Drive file ID, kept purely so repeat
  visits reuse the same folder instead of creating a new one each time. It
  isn't a credential.
- **Every Drive call is a plain `fetch`** against the REST v3 API
  (`src/drive/api.ts`). There's no Google API client library involved, so
  there's nothing to audit beyond what's in this repo.
...
1. In [Google Cloud Console](https://console.cloud.google.com/apis/credentials),
   create (or reuse) a project, then go to **Credentials**, then
   **Create Credentials**, then **OAuth client ID**, and choose
...
4. Open the deployed site and go to the **Vault** tab, then **Settings**
   (gear icon), paste in the Client ID, and hit **Save**. It's stored in
...
triage (§5 through §14) covering correctness, memory, lifecycle, test
coverage, and input-validation fixes.

This is a browser control surface for the **imaging** image/PDF engine.
It's a separate app: imaging is consumed as an ordinary dependency
...
- **Resize**: exact target dimensions, with 8 resampling kernels (triangle,
...
- **Crop**: fill a target width/height with a center/top/bottom/left/right
  anchor (cover-crop, done directly on decoded pixels). It re-encodes in
  whatever format the input already was; cropping never forces a format
  change, regardless of whether Compress is also enabled (see CHANGELOG
  §9). That's not the same as lossless, though: PNG/BMP/TGA re-encode
  losslessly, but JPEG and WebP crops are still a same-format re-encode at
  a fixed quality (q95), an extra, incidental generation of lossy
  compression on top of whatever the source already had (see CHANGELOG
  §13)
- **Compress**: imaging's bisection search for a target file size, using
  either the WASM (libjpeg-turbo/jpegopt) or zero-WASM Canvas engine, with
  a quality floor, max scale-down attempts, chroma subsampling, progressive
...
- **Final output**: leave as-is, lay out on an A4 canvas at 300dpi for
  print (sizing, margins, alignment, quality), or bind into a single-page
  PDF
...
This runs all three Node-side suites, and none of them need a browser:

- `npm run pipeline-test` runs unit tests for the app's own pipeline logic
  in `src/state/pipeline.ts` (`buildPipelineConfig`, `isNoopConfig`,
  `needsCrop`, `hidesSizeReduction`, `targetLocksFormat`). It runs directly
  against the TypeScript source via Node 22's built-in
  `--experimental-strip-types`, so no build step is required. This is what
  actually encodes the Simple UI's product semantics (for example, what
  "Same as original" plus "Shrink file size" produces for a PNG upload),
  so it's covered independently of the engine itself.
- `npm run client-test` runs regression tests for `src/engine/client.ts`'s
  worker lifecycle (crash detection, `restart()`, `cancel()`), against the
  real, unmodified `EngineClient` with a fake `Worker` that implements the
  same `onmessage`/`onerror`/`postMessage`/`terminate` contract a real one
...
- `npm run smoke-test` exercises every single imaging call the worker
  ...
  page-count plus page-extraction path) against the real pinned WASM
  binaries, using imaging's Node codec variant. Node has no
  `OffscreenCanvas`, so this covers everything except the Canvas compress
  engine, which only runs in an actual browser.
...
src/
  engine/
    ...
    client.ts                main-thread RPC wrapper plus live log/progress event stream
  components/                one component per pipeline stage / UI concern
  state/pipeline.ts          default pipeline config + formatting helpers
  App.tsx                    wires upload, then PDF extraction, then pipeline, then result
scripts/
...
passes searching for a target size, and PDF page rendering can be
...
files (never bundled/inlined) so they can be `fetch()`-ed at a known URL.
`scripts/copy-wasm-assets.mjs` copies them from `node_modules` into
...
There's nothing in `package.json` (no `repository`/`homepage` field) this
could be inferred from automatically, so it isn't. Leaving
