/**
 * Node-only codec wiring, for the smoke test suite ONLY.
 *
 * This exists because jSquash's own docs describe Node support as
 * "limited/experimental" (migration_v2.md §4.1) — their default `init()`
 * paths try to `fetch()` a same-directory wasm URL, which doesn't resolve
 * under plain `file://` Node execution. Passing each codec's `init()` an
 * explicit `wasmBinary` read via `fs.readFileSync` (confirmed working
 * against the real packages during this migration) sidesteps that without
 * needing a real browser — which is what migration_v2.md flagged as the
 * actual gap ("validating this integration will need a real browser").
 *
 * Production code should use `src/browser-codecs.ts` instead, which fetches
 * these same pinned assets over HTTP the way a real deployed page would.
 */
